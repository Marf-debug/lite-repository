...
# connection to DB
def return_cohort(username=None, password=None, cohort_type=0, dataset_path=None):
    """Load the cohort from DB (if creds provided) or from a local CSV (default).

    Using the local CSV avoids network/credential issues and keeps the notebook reproducible.
    """
    if dataset_path is None:
        default_paths = [
            "/mnt/data/t2dm_synthetic_T2DM_only (3).csv",
            "/mnt/data/t2dm_synthetic_T2DM_only.csv",
            "t2dm_synthetic_T2DM_only.csv",
        ]
    ...
    if username is None or password is None:
        if dataset_path is None:
            raise FileNotFoundError("No dataset_path provided and no default dataset file found.")
        ...
        POSTGRES_PASSWORD = urlquote(password)
...
def dataframe_2_qgrid(df):
    if qgrid is None:
        raise ImportError('qgrid is not installed or failed to import. Install qgrid or skip qgrid display.')
...
def infer_var_type(s: pd.Series) -> str:
    """Infer variable type for EDA outputs."""
    ...
    if pd.api.types.is_numeric_dtype(s):
        ...
        return 'Boolean' if nunq <= 2 else 'Numerical'
    return 'Categorical'
...
def features_description(dataframe, category):
    ...
    for feature in dataframe.columns:
        ...
        feature_type.append(infer_var_type(dataframe[feature]))
