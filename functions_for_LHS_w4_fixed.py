...
# =========================
# PATCH (2026): make modelling + checkboxes robust to dataset schema changes
# - safer feature selection (no hard-coded positional indices)
# - stable one-hot feature names using input_features
# - checkboxes2features works with dynamic checkbox UIs
# =========================
...
def cohort_2_transform_df(X_train, X_test, scaler = True):
    ...
    - categorical (object dtype): one-hot encode with stable names like <col>_<category>
    Returns numpy + DataFrame versions.
    """
    values_num = X_train.dtypes != object
    values_cat = X_train.dtypes == object
    ...
    # names
    names_num = [name for name, is_num in num_vars.items() if is_num]

    names_cats = []
    cat_feature_names = [name for name, is_cat in cat_vars.items() if is_cat]
    if len(cat_feature_names) > 0:
        pipe_cats_actual = col_transformer.named_transformers_['cats']
        # stable names: e.g. sex_F, smoker_CURRENT, method_manage_dt2_TABLETS
        names_cats = list(pipe_cats_actual['ohe'].get_feature_names_out(cat_feature_names))
...
def checkboxes2features(checkboxes):
    """Extract selected features from a list of HBox rows of checkboxes.
    Works with dynamically generated checkbox grids.
    """
    features_list = []
    for row in checkboxes:
        # row is usually an ipywidgets HBox
        children = getattr(row, 'children', [])
        for cb in children:
            if getattr(cb, 'value', False):
                features_list.append(getattr(cb, 'description', ''))

    # Remove master toggle if present
    features_list = [f for f in features_list if f and f.lower() != 'all']
    return features_list


def _top_k_features(model, X_train_df, y_train, k):
    """Return list of top-k feature names for a fitted model, robust across model types."""
    k = int(max(1, min(k, X_train_df.shape[1])))

    name = model.__class__.__name__
    # Fit a fresh clone so we don't mutate callers' estimator
    m = clone(model)
    ...
    if name == 'LogisticRegression':
        ...
        return list(X_train_df.columns[top_idx])
    ...
    if name == 'DecisionTreeClassifier':
        ...
        if imps is None:
            ...
            mi = mutual_info_classif(X_train_df.values, y_train, discrete_features=False, random_state=0)
            top_idx = np.argsort(mi)[::-1][:k]
        else:
            top_idx = np.argsort(imps)[::-1][:k]
        return list(X_train_df.columns[top_idx])

    # Generic fallback: mutual information
    mi = mutual_info_classif(X_train_df.values, y_train, discrete_features=False, random_state=0)
    top_idx = np.argsort(mi)[::-1][:k]
    return list(X_train_df.columns[top_idx])


def modelling_base_vs_best(model, X_train, X_test, y_train, y_test, k_best=None):
    """Compare:
    - model trained on all features
    - model trained on top-k automatically selected features
    Avoids hard-coded positional index dicts (prevents IndexError).
    """
    # choose k
    if k_best is None:
        if model.__class__.__name__ == 'LogisticRegression':
    ...
    y_pred = m_base.predict(X_test)

    cv_roc_auc_mean,cv_mcc_mean,cv_accuracy_mean,cv_f1_mean,cv_precision_mean,cv_recall_mean, roc_auc,mcc,accuracy,f1,precision,recall = return_scores(m_base, X_train, X_test, y_train, y_test)

    # pick best features
    best_feats = _top_k_features(model, X_train, y_train, k_best)
    X_train_best = X_train[best_feats]
    X_test_best  = X_test[best_feats]

    # best model
    m_best = clone(model)
    m_best.fit(X_train_best, y_train)
    y_pred_best = m_best.predict(X_test_best)

    cv_roc_auc_mean_best,cv_mcc_mean_best,cv_accuracy_mean_best,cv_f1_mean_best,cv_precision_mean_best,cv_recall_mean_best, roc_auc_best, mcc_best, accuracy_best, f1_best, precision_best, recall_best = return_scores(m_best, X_train_best, X_test_best, y_train, y_test)

    # plots
    if (model.__class__.__name__=='LogisticRegression'):
        fig, ax = plt.subplots(1,4, figsize=(27,5))
        fig.suptitle(f'MODEL WITH ALL FEATURES (n={X_train.shape[1]})              MODEL WITH TOP-{len(best_feats)} FEATURES', y=1.09, x=0.55, fontsize=18, fontweight='bold')
    else:
        fig, ax = plt.subplots(1,4, figsize=(27,5))
        fig.suptitle(f'MODEL WITH ALL FEATURES (n={X_train.shape[1]})              MODEL WITH TOP-{len(best_feats)} FEATURES', y=1.09, x=0.55, fontsize=18, fontweight='bold')

    RocCurveDisplay.from_estimator(m_base, X_test, y_test, ax=ax[0])
    ax[0].plot([0,1],[0,1], linestyle='--', lw=2, label='Chance', alpha=.8)
    sns.heatmap(confusion_matrix(y_test, y_pred, labels=[1,0]), annot=True, fmt='g', ax=ax[1], cmap='Blues',
                xticklabels=['admitted','non-admitted'], yticklabels=['admitted','non-admitted'])
    ax[1].set_ylabel('Actual'); ax[1].set_xlabel('Predicted')

    create_table_with_metrics_n_column(
        [[cv_roc_auc_mean,roc_auc,cv_roc_auc_mean_best,roc_auc_best],
         [cv_accuracy_mean,accuracy,cv_accuracy_mean_best,accuracy_best],
         [cv_precision_mean,precision,cv_precision_mean_best,precision_best],
         [cv_recall_mean,recall,cv_recall_mean_best,recall_best]],
        f"{model.__class__.__name__} (all vs top-{len(best_feats)})",
        4
    )

    RocCurveDisplay.from_estimator(m_best, X_test_best, y_test, ax=ax[2])
    ax[2].plot([0,1],[0,1], linestyle='--', lw=2, label='Chance', alpha=.8)
    sns.heatmap(confusion_matrix(y_test, y_pred_best, labels=[1,0]), annot=True, fmt='g', ax=ax[3], cmap='Oranges',
                xticklabels=['admitted','non-admitted'], yticklabels=['admitted','non-admitted'])
    ax[3].set_ylabel('Actual'); ax[3].set_xlabel('Predicted')
    plt.show()

    # Feature importance bar chart (when available)
    if model.__class__.__name__ in ('LogisticRegression','DecisionTreeClassifier'):
        fig, axp = plt.subplots(figsize=(max(12, min(24, len(best_feats)*0.8)), 5))
        if model.__class__.__name__=='LogisticRegression':
            coef = np.abs(m_best.coef_[0])
            vals = coef / (coef.sum() if coef.sum()!=0 else 1)
        else:
            vals = getattr(m_best, 'feature_importances_', np.ones(len(best_feats))/len(best_feats))
        imp_df = pd.DataFrame({'features': best_feats, 'importance': vals}).sort_values('importance', ascending=False)
        plt.bar(imp_df['features'], imp_df['importance'], align='center', alpha=0.7)
        plt.xticks(rotation=80)
        plt.ylabel('Importance')
        plt.title(f'Top features for {model.__class__.__name__}')
        plt.show()
